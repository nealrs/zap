...
**Author**: Neal Shyam (@nealrs)  
**Copyright**: © 2026 Neal Shyam. All rights reserved.  
**Links**: [nealshyam.com](https://nealshyam.com) | [GitHub](https://github.com/nealrs)
...
│   │   ├── icon.svg       # Source icon for all sizes
│   │   ├── icon-*.png     # Generated icons (16x16 to 512x512)
│   │   └── manifest.json  # PWA manifest for installation
│   ├── index.html         # Main game page
│   └── dev.html           # Developer level selector
...
### Player Features
- **Level Selector**: Choose any unlocked level (unlock by completing previous levels)
- **Progress Tracking**: Game saves your progress via localStorage
- **Loading Screen**: 5-second branded splash screen (shown once per day)
- **PWA Support**: Install as a standalone app on mobile/desktop
- **Offline Mode**: Full offline gameplay after first load
- **Responsive Design**: Optimized for mobile (portrait) and desktop
- **Credits**: Author attribution on loading, level selector, and win screens

### Developer Features
- **Dev Mode**: Access via `/dev` route - test any level directly
- **Level Editor**: Modify `levels.json` to create/edit levels
- **Finale Test**: Test victory screen via `/dev/finale`
- **Icon Generation**: Auto-generate all PWA icons from SVG source
...
### Deploy as PWA
1. Host on HTTPS-enabled server
2. Service worker auto-registers
3. Users can install via browser prompt

## User Interface & Navigation

### Normal Player Flow
1. **Loading Screen** (5 seconds, once per day)
   - Shows game logo and credits
   - Auto-hides after minimum display time
   
2. **Level Selector**
   - Grid of all levels (2 columns on mobile)
   - Locked levels (🔒) until previous level completed
   - Unlocked levels (🔓) are playable
   - Credits footer with links
   - Scrollable when many levels
   
3. **Level Intro**
   - Shows level name and description
   - "START LEVEL" button (primary)
   - "All Levels" button (secondary) - return to selector
   
4. **Gameplay**
   - HUD shows: Level, Pops, Time
   - 3D environment with bubbles and hazards
   - Pop bubbles by tapping/clicking
   
5. **Level Complete**
   - Shows victory/failure message
   - "NEXT LEVEL" button (primary) - continue sequential play
   - "All Levels" button (secondary) - return to selector
   - Automatically unlocks next level on victory
   
6. **Game Complete** (after final level)
   - Colorful finale screen with credits
   - "🎮 PLAY AGAIN 🎮" - restart from level 1
   - "All Levels" - return to selector

### Developer Flow
1. Visit `/dev` for dev menu
2. **Dev Level Selector**
   - Grid of all levels (red/orange theme)
   - All levels unlocked
   - Direct level testing
   - "TEST FINALE SCREEN" button
   
3. **Level Intro** (Dev Mode)
   - "START LEVEL" button (primary)
   - "← Dev Menu" button (secondary) - return to `/dev`
   
4. **Level End** (Dev Mode)
   - Shows pass/fail and timing info
   - "← BACK TO DEV MENU" button
...
**Bumper** - Bounces bubbles with enhanced force (NEW)
...
## Special Bubbles Configuration (specialbubbles.json)

The `specialbubbles.json` file defines power-up and bonus bubble types. Available special bubbles:
...
To add a new special bubble, add it to `specialbubbles.json`:
...
### Loading Screen
- Animated gradient logo with rainbow shimmer
- Rotating spinner
- Credits with clickable links
- 5-second minimum display (once per day via localStorage)
- Fade-out transition (600ms)

### Level Selector
- 2-column grid on mobile, scrollable
- Gradient blue buttons for unlocked levels
- Gray disabled buttons for locked levels
- Hover effects on unlocked levels
- Credits footer with links
- Responsive padding and margins

### Finale Screen
- Colorful radial gradients (purple, blue, gold)
- Animated scanlines and pulsing effects
- Victory celebration styling
- Game completion message with credits

### Visual Effects
- **Reference Dots**: Spherical grid of dots for depth perception
  - Medium gray (0x666666), size 1.0, opacity 0.6
  - Latitude/longitude grid on 35-unit radius sphere
- **Starfield**: 800 dim background stars for space atmosphere
- **Bubble Effects**: Iridescent shading with normal maps
- **Hazard Visuals**: 
  - Wormhole: Purple glowing torus knot
  - Repulsor: Orange wireframe icosahedron
  - Bumper: Yellow glowing dodecahedron
...
### Button Management
- Dynamic button creation/removal on navigation
- `querySelectorAll` cleanup to prevent duplicates
- Consistent styling across normal/dev modes
- Primary CTAs are large and prominent
- Secondary options are smaller and subdued

### Scrolling & Layout
- Overlay uses `flex-start` alignment for proper scrolling
- `overflow-y: auto` on overlay container
- `margin: auto 0` for vertical centering when space allows
- No scroll constraints on level selector grid
- Mobile-optimized spacing and padding

### Performance
- Efficient bubble respawning (never exceed count limit)
- Physics calculations optimized for mobile
